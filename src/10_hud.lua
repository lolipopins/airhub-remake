--// AirHub - 10_hud.lua
--// Pretty HUD (rounded background + accent bar + stats) injected into Settings tab.
...
local function update()
    ...
    --// Line 1 — Players / FPS
    local parts1 = {}
    if HUD.Settings.ShowPlayers then
        table.insert(parts1, string.format("Players  %d", #Players:GetPlayers()))
    end
    if HUD.Settings.ShowFPS then
        table.insert(parts1, string.format("FPS  %d", HUD.Internal.CurrentFps))
    end
...
HUD.Functions.Stop = function()
    if HUD.Internal.Conn then
        HUD.Internal.Conn:Disconnect()
        HUD.Internal.Conn = nil
    end
    hideAll()
    HUD.Internal.Visible = false
end
...
//// UI — Settings tab
--// ---------------------------------------------------------------------------
local function fillSettingsUI()
    local tab = H._UI and H._UI.SettingsTab
    if not tab then return false end

    local secH = tab:CreateSection({ Name = "HUD" })
    secH:AddToggle({
        Name = "Enable HUD",
        Value = HUD.Settings.Enabled,
...
if not fillSettingsUI() then
    task.spawn(function()
        for _ = 1, 60 do
            if H.ShuttingDown then return end
            if fillSettingsUI() then return end
            task.wait(0.5)
        end
        warn("[AirHub] 10_hud: SettingsTab not found after 30s")
    end)
end
